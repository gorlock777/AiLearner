...
export function LandingPage() {
  ...
  // Fluid, free-scrolling parallax (no sticky pin locking)
  const { scrollY } = useScroll()
  const blackHoleScale = useTransform(scrollY, [0, 800], [1, 1.3])
  const blackHoleY = useTransform(scrollY, [0, 800], [0, 180])
  ...
  return (
    <div className="min-h-screen bg-[#070709] text-zinc-100 flex flex-col selection:bg-zinc-800 selection:text-white linear-grid overflow-x-hidden">
      ...
      {/* ── Fluid Free-Scrolling Black Hole Hero Section ── */}
      <section
        ref={heroRef}
        className="relative min-h-screen w-full flex items-center justify-center overflow-hidden bg-black pt-20 pb-16 px-6"
      >
        {/* Background 3D Black Hole Canvas with Fluid Parallax */}
        <motion.div
          style={{
        ...
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-0 bg-radial from-black/10 via-black/55 to-[#070709]"
        />

        {/* Hero Content with Free Parallax */}
        <motion.div
          style={{
            y: heroContentY,
        ...
        >
          {/* Subtle Top Badge */}
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-zinc-950/80 border border-white/10 text-[11px] font-mono text-zinc-300 mb-6 backdrop-blur-md shadow-2xl">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
            <span>The Singularity of Learning · OpenRouter Free</span>
          </div>

          {/* Giant Grand Merriweather Headline */}
          <h1 className="font-heading font-normal text-4xl sm:text-6xl md:text-7xl lg:text-[5.2rem] leading-[1.05] tracking-tight text-white mb-6 drop-shadow-2xl">
          ...
          <p className="max-w-xl text-sm sm:text-base md:text-lg text-zinc-300 leading-relaxed font-sans mb-8">
            AiLearner transforms complex study materials into active recall flashcards, 3D memory physics, and diagnostic test feedback.
          </p>
      ...
      <footer className="py-12 border-t border-white/[0.06] bg-[#070709] text-center text-xs font-mono text-zinc-500 relative z-10">
        <div className="max-w-6xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div>MIT License · High-Performance Client Engine</div>
        </div>
      </footer>
    </div>
  )
